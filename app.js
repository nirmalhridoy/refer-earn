...
const fbHistoryToggle = $("fbHistoryToggle");
const fbHistoryList = $("fbHistoryList");
const fbHistoryCount = $("fbHistoryCount");
const fbHistoryArrow = $("fbHistoryArrow");
const ytHistoryToggle = $("ytHistoryToggle");
const ytHistoryList = $("ytHistoryList");
const ytHistoryCount = $("ytHistoryCount");
const ytHistoryArrow = $("ytHistoryArrow");
...
function attachWithdrawListener(uid) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    if (docs.length > 0) {
      renderWithdrawTimeline(docs[0].status);
    } else {
      renderWithdrawTimeline(null);
    }
  });
  subscriptions.push(unsub);
}
...
function notificationCategory(n) {
  ...
  if (title.includes("bonus") || title.includes("commission") || title.includes("income") || title.includes("reward")) return "earnings";
  return "system";
}
...
function renderNotifications() {
  ...
  if (currentNotifFilter === "unread") {
    ...
    filtered = allNotifications.filter((n) => notificationCategory(n) === currentNotifFilter);
  }
...
function attachNotificationsListener(uid) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    allNotifications = [];
    snap.forEach((d) => allNotifications.push(d.data()));
    renderNotifications();
  });
  subscriptions.push(unsub);
}
...
function renderTaskSection(platform) {
  ...
  container.innerHTML = list.length
    ...
    : `<div class="card">📋 No Available Tasks</div>`;
...
function renderTaskHistory() {
  ...
  const fbCompleted = completed.filter((s) => s.platform === "facebook");
  const ytCompleted = completed.filter((s) => s.platform === "youtube");

  fbHistoryCount.textContent = fbCompleted.length;
  ytHistoryCount.textContent = ytCompleted.length;

  fbHistoryList.innerHTML = fbCompleted.length
    ? fbCompleted.map((s) => `
    ...
    : `<div class="task-history-item">এখনো কোনো Facebook Task Approved হয়নি।</div>`;

  ytHistoryList.innerHTML = ytCompleted.length
    ? ytCompleted.map((s) => `
      <div class="task-history-item">
        <strong>${s.taskName}</strong>
        Reward: ${formatCurrency(s.reward)} • Submitted: ${formatDate(s.submittedAt)} • Approved: ${formatDate(s.reviewedAt)}
      </div>`).join("")
    : `<div class="task-history-item">এখনো কোনো YouTube Task Approved হয়নি।</div>`;
}

fbHistoryToggle.addEventListener("click", () => {
  fbHistoryList.classList.toggle("hidden");
  fbHistoryArrow.classList.toggle("arrow-open");
});

ytHistoryToggle.addEventListener("click", () => {
  ytHistoryList.classList.toggle("hidden");
  ytHistoryArrow.classList.toggle("arrow-open");
});
