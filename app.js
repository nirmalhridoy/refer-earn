...
function attachTodayIncomeListener(uid) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    snap.forEach((d) => {
      const t = d.data();
      if (["referral_bonus", "activation_bonus", "admin_credit", "daily_login_bonus", "lucky_spin", "rank_bonus", "facebook_task_reward", "youtube_task_reward", "contest_prize"].includes(t.type)) {
