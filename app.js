...
function renderUserData(data) {
  ...
  dailyBonusStatusEl.textContent = data.lastLoginBonusDate === today
    ? "✅ আজকের ৳1 Bonus সংগ্রহ করা হয়েছে।"
    : "প্রতিদিন Login করলে ৳1 Bonus স্বয়ংক্রিয়ভাবে যোগ হয়।";
...
function attachTodayIncomeListener(uid) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    snap.forEach((d) => {
      const t = d.data();
      if (["referral_bonus", "activation_bonus", "admin_credit", "daily_login_bonus", "lucky_spin"].includes(t.type)) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    activityListEl.innerHTML = docs.length
      ? docs.slice(0, 5).map((t) => `
        <div class="card">
          <strong>${t.type.replace("_", " ")}</strong> — ${Number(t.amount) >= 0 ? "+" : ""}${formatCurrency(t.amount)}
...
editProfileBtn.addEventListener("click", () => {
  ...
  editPhotoEl.value = currentUserData.photoURL || "";
...
saveProfileBtn.addEventListener("click", async () => {
  ...
  const photoURL = editPhotoEl.value.trim();
  ...
  try {
    ...
    await updateDoc(doc(db, USERS, currentUser.uid), { name, phone, district, photoURL });
...
withdrawBtn.addEventListener("click", async () => {
  if (!currentUserData.isActive) {
    showWarning("Please activate your account before requesting a withdrawal.");
    return;
  }
  ...
  if (!amount || amount <= 0) {
    showWarning("Please enter a valid withdraw amount.");
    return;
  }
  if (!accountNumber) {
    showWarning("Please enter your account number.");
    return;
  }
  ...
  const confirmed = await showConfirm("Confirm Withdraw", `Withdraw ${formatCurrency(amount)} to ${accountNumber}?`);
  if (!confirmed) return;
  ...
  try {
    ...
    showSuccess("Withdraw request submitted successfully.");
  } catch (err) {
    showError(err.message);
  } finally {
    showActionLoading(false);
...
spinBtn.addEventListener("click", async () => {
  ...
  const segmentCenter = rewardIndex * 90 + 45;
    ...
    try {
      ...
      const granted = await runTransaction(db, async (transaction) => {
        ...
        transaction.set(notifRef, {
          uid: currentUser.uid,
          title: "Lucky Spin",
          message: `Lucky Spin থেকে আপনি ৳${reward} জিতেছেন!`,
          type: "success",
          isRead: false,
          createdAt: serverTimestamp()
        });
      ...
      if (granted) {
        spinResultEl.textContent = `🎉 আপনি ৳${reward} জিতেছেন!`;
