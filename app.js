...
(function injectPremiumStyles() {
  ...
  style.textContent = `
    ...
    .task-filter-bar input, .task-filter-bar select {
      height: 46px; padding: 0 14px; background: var(--bg2);
...
function renderWelcomeHeader(data) {
  ...
  const goalText = rankInfo.isMax
    ...
    : `🎯 আরও ${rankInfo.next.minReferrals - activeCount}টি Active Referral লাগবে ${rankInfo.next.name} Rank-এ যেতে`;
...
function renderActiveTaskList() {
  ...
  // Approved tasks are excluded from the active marketplace list — they only live in Task History
...
function updateAvailableTaskStat() {
  ...
  allActiveTasks.forEach((t) => {
    const sub = mySubmissions[t.id];
    if (!sub || sub.status === "rejected") count++;
  });
  statAvailableTasksEl.textContent = count;
}
...
function updateWithdrawUI() {
  ...
  if (fullyEligible) {
    ...
    if (!tasksMet) missing.push("New Tasks Completed");
...
function attachTransactionsListener(uid) {
  ...
  const q = query(collection(db, TRANSACTIONS), where("uid", "==", uid), orderBy("createdAt", "desc"), limit(200));
  const unsub = onSnapshot(q, (snap) => {
    const docs = [];
