...
function attachTodayIncomeListener(uid) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    snap.forEach((d) => {
      const t = d.data();
      if (["referral_bonus", "activation_bonus", "admin_credit"].includes(t.type)) {
...
activationBtn.addEventListener("click", async () => {
  if (currentUserData.isActive) {
    showWarning("Your account is already active.");
    return;
  }
  if (pendingActivationCount > 0) {
    showWarning("You already have a pending activation request.");
    return;
  }
  ...
  if (!senderNumber || !transactionId) {
    showWarning("Please fill in all activation fields.");
    return;
  }
  ...
  try {
    ...
    showSuccess("Activation request submitted. Await admin approval.");
  } catch (err) {
    showError(err.message);
  } finally {
    showActionLoading(false);
