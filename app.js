...
const dashAvailableBalance = $("dashAvailableBalance");
const dashLifetimeIncome = $("dashLifetimeIncome");
const dashTodayEarnings = $("dashTodayEarnings");
const dashWeekEarnings = $("dashWeekEarnings");
const dashMonthEarnings = $("dashMonthEarnings");
const dashReferralEarnings = $("dashReferralEarnings");
const dashTaskEarnings = $("dashTaskEarnings");
const dashBonusEarnings = $("dashBonusEarnings");
const dashTotalWithdraw = $("dashTotalWithdraw");
const dashPendingWithdrawAmount = $("dashPendingWithdrawAmount");

const totalReferralsEl = $("totalReferrals");
const statActiveReferralsEl = $("statActiveReferrals");
const statPendingReferralsEl = $("statPendingReferrals");
const statWithdrawAvailableReferralsEl = $("statWithdrawAvailableReferrals");
const approvedTaskStatEl = $("approvedTaskStat");
const statAvailableTasksEl = $("statAvailableTasks");
...
const taskSubmitRejectReasonBox = $("taskSubmitRejectReasonBox");
...
const rejectedTaskContainer = $("rejectedTaskContainer");
...
const BONUS_TYPES = ["daily_login_bonus", "lucky_spin", "registration_bonus", "activation_bonus", "rank_bonus", "admin_credit"];
const REFERRAL_TYPES = ["referral_bonus"];
const TASK_TYPES = ["task_reward"];
...
(function injectPremiumStyles() {
  ...
  style.textContent = `
    @keyframes premiumShimmer { 0% { background-position: -400px 0; } 100% { background-position: 400px 0; } }
    .skeleton-line { height: 14px; border-radius: 6px; margin-bottom: 10px;
      ...
      background-size: 800px 100%; animation: premiumShimmer 1.4s ease-in-out infinite; }
    .skeleton-line:last-child { margin-bottom: 0; }
    .skeleton-card { background: var(--card); border: 1px solid var(--border); border-radius: var(--radius); padding: 18px; margin-bottom: 12px; }
    .premium-welcome-banner { background: linear-gradient(135deg, rgba(37,99,235,.18), rgba(56,189,248,.10));
      border: 1px solid rgba(56,189,248,.3); border-radius: var(--radius); padding: 18px 22px; margin-bottom: 20px;
      display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px; }
    .premium-welcome-banner h3 { font-size: 17px; font-weight: 700; margin-bottom: 4px; }
    .premium-welcome-banner p { font-size: 12.5px; color: var(--text-light); }
    .premium-welcome-emoji { font-size: 28px; }
    .task-filter-bar { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 18px; }
    .task-filter-bar input, .task-filter-bar select { height: 46px; padding: 0 14px; background: var(--bg2);
    ...
    .wallet-group-title { font-size: 12px; font-weight: 700; color: var(--text-muted); text-transform: uppercase; letter-spacing: .5px; margin: 18px 0 10px; }
    .wallet-group-title:first-child { margin-top: 0; }
    .rank-remaining-text { margin-top: 10px; font-size: 13px; color: var(--secondary); font-weight: 600; }
...
function renderWelcomeHeader(data) {
  ...
  const activeCount = Number(statActiveReferralsEl.textContent) || 0;
  const rankInfo = getRankInfo(activeCount);
  const goalText = rankInfo.isMax
    ? `🏆 Congratulations! You have reached ${rankInfo.current.name} Rank.`
    : `🎯 ${rankInfo.next.name} Rank পেতে আরও ${rankInfo.next.minReferrals - activeCount}টি Active Referral প্রয়োজন।`;
...
function renderActiveTaskList() {
  ...
  let list = allActiveTasks.filter((t) => {
    if (t.platform !== activeCategory) return false;
  ...
  allActiveTasks.filter((t) => t.platform === activeCategory).forEach((t) => {
    const sub = mySubmissions[t.id];
    if (!sub || sub.status === "rejected") availableCount++;
  });
  ...
  taskCategoryCounter.textContent = availableCount;
...
function renderRejectedTasks() {
  const rejected = Object.values(mySubmissions).filter((s) => s.status === "rejected");

  rejectedTaskContainer.innerHTML = rejected.length
    ? rejected.map((s) => {
        const platformInfo = TASK_PLATFORMS.find((p) => p.key === s.platform) || { icon: "📋", label: s.platform };
        return `<div class="rejected-task-card">
          <h4>${platformInfo.icon} ${s.taskName}</h4>
          <p><strong>Rejected কারণ:</strong> ${s.rejectReason || "কোনো কারণ উল্লেখ করা হয়নি।"}</p>
          <p><strong>Submitted:</strong> ${formatDate(s.submittedAt)}</p>
          <button type="button" class="btn btn-primary btn-xs rejected-resubmit-btn" data-task-id="${s.taskId}">আবার Submit করুন</button>
        </div>`;
      }).join("")
    : `<div class="card">এই মুহূর্তে কোনো Rejected Task নেই।</div>`;

  rejectedTaskContainer.querySelectorAll(".rejected-resubmit-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const task = allActiveTasks.find((t) => t.id === btn.dataset.taskId);
      if (task) openTaskSubmitModal(task);
    });
...
function openTaskSubmitModal(task) {
  ...
  if (sub && sub.status === "rejected" && sub.rejectReason) {
    taskSubmitRejectReasonBox.innerHTML = `<strong>⚠️ পূর্বে Rejected হয়েছিল</strong>কারণ: ${sub.rejectReason}`;
    taskSubmitRejectReasonBox.classList.remove("hidden");
  } else {
    taskSubmitRejectReasonBox.classList.add("hidden");
  }
...
taskScreenshotInput.addEventListener("change", () => {
  ...
  if (!file) return;
  if (!file.type.startsWith("image/")) { showWarning("সঠিক Image ফাইল Select করুন।"); return; }
...
taskSubmitBtn.addEventListener("click", async () => {
  ...
  if (!task) { showError("Task খুঁজে পাওয়া যায়নি।"); return; }
  if (!selectedTaskScreenshotBase64) { showWarning("Screenshot Upload করা বাধ্যতামূলক।"); return; }
  if (!profileLink) { showWarning("Profile Link অথবা User ID দিন।"); return; }
...
function updateAvailableTaskStat() {
  if (!currentUserData) return;
  statAvailableTasksEl.textContent = currentUserData.cycleTaskCount || 0;
}
...
phoneSignupBtn.addEventListener("click", async () => {
  ...
  if (!BD_PHONE_REGEX.test(phone)) { showWarning("সঠিক ১১ ডিজিটের মোবাইল নাম্বার দিন (যেমন 01XXXXXXXXX)।"); return; }
  if (!password || password.length < 6) { showWarning("Password কমপক্ষে ৬ অক্ষরের হতে হবে।"); return; }
...
phoneLoginBtn.addEventListener("click", async () => {
  ...
  if (!BD_PHONE_REGEX.test(phone)) { showWarning("সঠিক ১১ ডিজিটের মোবাইল নাম্বার দিন (যেমন 01XXXXXXXXX)।"); return; }
  if (!password) { showWarning("Password দিন।"); return; }
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      transaction.set(txnRef, {
        uid, type: "registration_bonus", amount: REGISTRATION_BONUS,
        description: "Registration Welcome Bonus", status: "completed", createdAt: serverTimestamp()
      });
      const notifRef = doc(collection(db, NOTIFICATIONS));
      transaction.set(notifRef, {
        uid, title: "Welcome Bonus",
        message: `Registration সফল হয়েছে! স্বাগতম বোনাস হিসেবে ৳${REGISTRATION_BONUS} আপনার Wallet-এ যোগ হয়েছে।`,
        type: "success", isRead: false, createdAt: serverTimestamp()
      });
    });
...
async function claimDailyLoginBonus(uid) {
  ...
  try {
    ...
    const claimed = await runTransaction(db, async (transaction) => {
      ...
      const txnRef = doc(collection(db, TRANSACTIONS));
      transaction.set(txnRef, {
        uid, type: "daily_login_bonus", amount: DAILY_LOGIN_BONUS,
        description: "Daily Login Bonus", status: "completed", createdAt: serverTimestamp()
      });
      const notifRef = doc(collection(db, NOTIFICATIONS));
      transaction.set(notifRef, {
        uid, title: "Daily Login Bonus",
...
function renderUserData(data) {
  ...
  dashAvailableBalance.textContent = formatCurrency(data.walletBalance);
  dashLifetimeIncome.textContent = formatCurrency(data.totalIncome);

  accountStatusEl.textContent = data.isFrozen ? "Frozen" : data.isActive ? "Active" : pendingActivationCount > 0 ? "Pending Activation" : "Inactive";
  ...
  statAvailableTasksEl.textContent = data.cycleTaskCount || 0;
...
function updateWithdrawUI() {
  ...
  const cycleTasks = currentUserData.cycleTaskCount || 0;

  withdrawWalletBalanceEl.textContent = formatCurrency(balance);
  withdrawApprovedTasksEl.textContent = `${cycleTasks} / ${WITHDRAW_REQUIRED_APPROVED_TASKS}`;
  ...
  const tasksPercent = Math.min(100, Math.round((cycleTasks / WITHDRAW_REQUIRED_APPROVED_TASKS) * 100));
  const tasksMet = cycleTasks >= WITHDRAW_REQUIRED_APPROVED_TASKS;
  $("eligTasksBar").style.width = tasksPercent + "%";
  $("eligTasksBar").textContent = tasksPercent + "%";
  $("eligTasksText").textContent = `${Math.min(cycleTasks, WITHDRAW_REQUIRED_APPROVED_TASKS)}/${WITHDRAW_REQUIRED_APPROVED_TASKS} ${tasksMet ? "✓" : ""}`;
  ...
  if (fullyEligible) {
    ...
    const missing = [];
    if (!balanceMet) missing.push(`Wallet Balance: ${formatCurrency(balance)}/৳${WITHDRAW_AMOUNT} ✕`);
    if (!tasksMet) missing.push(`New Tasks: ${cycleTasks}/${WITHDRAW_REQUIRED_APPROVED_TASKS} ✕`);
    if (condition12Met && !referralMet) missing.push(`New Active Referrals: ${availableReferral}/${REQUIRED_NEW_REFERRAL} ✕`);
    finalStatusEl.className = "eligibility-final-status status-not-eligible";
    finalStatusEl.innerHTML = `🔒 Not Eligible Yet<br><span style="font-size:12px">${missing.join(" • ")}</span>`;
  }

  withdrawFormCard.classList.toggle("hidden", !fullyEligible);
  statWithdrawAvailableReferralsEl.textContent = availableReferral;
}

function renderWithdrawTimeline(status) {
  if (!status) { withdrawTimelineCard.classList.add("hidden"); return; }
  ...
  if (status === "pending") { activeSteps = 2; statusText = "⏳ আপনার Withdraw Request Admin যাচাই করছে।"; }
  else if (status === "approved") { activeSteps = 4; statusText = "✅ আপনার Withdraw সফলভাবে সম্পন্ন হয়েছে।"; }
  else if (status === "rejected") { activeSteps = 1; statusText = "❌ আপনার Withdraw Request Reject হয়েছে। Amount ফেরত দেওয়া হয়েছে।"; }
...
function attachReferralListener(referralCode) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    statActiveReferralsEl.textContent = activeCount;
    statPendingReferralsEl.textContent = snap.size - activeCount;
...
function withdrawStatusLabel(status) {
  if (status === "approved") return { text: "✅ Approved / Completed", cls: "badge-success" };
...
function sortByCreatedAtDesc(arr, field) {
  return [...arr].sort((a, b) => {
    const aT = a[field] && a[field].toMillis ? a[field].toMillis() : 0;
    const bT = b[field] && b[field].toMillis ? b[field].toMillis() : 0;
    return bT - aT;
  });
}
...
function renderWalletTimeline(docs) {
  ...
  docs.slice(0, 20).forEach((t) => { groups[getDateBucket(t.createdAt)].push(t); });
...
function computeDashboardEarnings(docs) {
  ...
  let today = 0, week = 0, month = 0, referralSum = 0, taskSum = 0, bonusSum = 0;
  ...
  docs.forEach((t) => {
    ...
    if (dateStr === todayStr) today += amount;
    if (date >= weekStart) week += amount;
    if (date >= monthStart) month += amount;

    if (REFERRAL_TYPES.includes(t.type)) referralSum += amount;
    else if (TASK_TYPES.includes(t.type)) taskSum += amount;
    else if (BONUS_TYPES.includes(t.type)) bonusSum += amount;
  });

  return { today, week, month, referralSum, taskSum, bonusSum };
}

function activityIcon(t) {
  const icons = {
    task_reward: "✅ Task Completed",
    referral_bonus: "👥 Referral Activated",
    daily_login_bonus: "🎁 Daily Bonus Received",
    lucky_spin: "🎁 Lucky Spin Bonus Received",
    registration_bonus: "🎉 Registration Bonus Received",
    activation_bonus: "🔐 Account Activated",
    rank_bonus: "🏆 Rank Bonus Received",
    admin_credit: "💰 Balance Added",
    withdraw: "📤 Withdrawal Submitted"
  };
  return icons[t.type] || `${t.type.replace(/_/g, " ")}`;
}
...
function attachTransactionsListener(uid) {
  ...
  // Fixed: removed orderBy() to avoid Firestore composite index requirement — sorting done client-side instead
  const q = query(collection(db, TRANSACTIONS), where("uid", "==", uid), limit(200));
  const unsub = onSnapshot(q, (snap) => {
    let docs = [];
    snap.forEach((d) => docs.push(d.data()));
    docs = sortByCreatedAtDesc(docs, "createdAt");

    const earnings = computeDashboardEarnings(docs);
    dashTodayEarnings.textContent = formatCurrency(earnings.today);
    dashWeekEarnings.textContent = formatCurrency(earnings.week);
    dashMonthEarnings.textContent = formatCurrency(earnings.month);
    dashReferralEarnings.textContent = formatCurrency(earnings.referralSum);
    dashTaskEarnings.textContent = formatCurrency(earnings.taskSum);
    dashBonusEarnings.textContent = formatCurrency(earnings.bonusSum);

    transactionHistoryEl.innerHTML = docs.length
      ? docs.slice(0, 50).map(transactionRow).join("")
...
function attachWithdrawListener(uid) {
  withdrawHistoryEl.innerHTML = skeletonTableRows(5, 3);

  // Fixed: removed orderBy() to avoid Firestore composite index requirement — sorting done client-side instead
  const q = query(collection(db, WITHDRAWS), where("uid", "==", uid), limit(50));
  const unsub = onSnapshot(q, (snap) => {
    let docs = [];
    snap.forEach((d) => docs.push(d.data()));
    docs = sortByCreatedAtDesc(docs, "requestedAt");
    ...
    dashTotalWithdraw.textContent = formatCurrency(totalWithdrawn);
    dashPendingWithdrawAmount.textContent = formatCurrency(totalPending);
    ...
    withdrawHistoryEl.innerHTML = docs.length
      ...
      : `<tr><td colspan="5">💸 No Withdrawal History Yet<br><span style="font-size:11px">Your withdrawal history will appear here.</span></td></tr>`;
...
function attachActivationListener(uid) {
  // Fixed: removed orderBy()+limit(1) combo (needed composite index) — fetch all, sort client-side
  const q = query(collection(db, ACTIVATIONS), where("uid", "==", uid));
  const unsub = onSnapshot(q, (snap) => {
    let docs = [];
    snap.forEach((d) => docs.push(d.data()));
    docs = sortByCreatedAtDesc(docs, "requestedAt");

    pendingActivationCount = docs.length > 0 && docs[0].status === "pending" ? 1 : 0;
    updatePendingRequests();

    if (currentUserData) {
      accountStatusEl.textContent = currentUserData.isFrozen ? "Frozen" : currentUserData.isActive ? "Active" : pendingActivationCount > 0 ? "Pending Activation" : "Inactive";
    }
  });
  subscriptions.push(unsub);
}
...
function attachLeaderboardListener() {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    leaderboardTableEl.innerHTML = docs.length
      ? docs.map((u, i) => `<tr><td>#${i + 1}</td><td>${u.name}</td><td>${u.activeReferralCount || 0}</td></tr>`).join("")
...
function mergeAndRenderNotifications() {
  ...
  let filtered = all;
  if (currentNotifFilter === "unread") filtered = all.filter((n) => !n.isRead);
  else if (currentNotifFilter !== "all") filtered = all.filter((n) => notificationCategory(n) === currentNotifFilter);

  notificationListEl.innerHTML = filtered.length
    ? filtered.map((n) => {
        const cat = notificationCategory(n);
        return `<div class="card"><strong>${categoryIcon(cat)} ${n.title}</strong><p>${n.message}</p><p>${formatDate(n.createdAt)}</p></div>`;
      }).join("")
...
function attachNotificationsListener(uid) {
  ...
  const personalUnsub = onSnapshot(query(collection(db, NOTIFICATIONS), where("uid", "==", uid), limit(50)), (snap) => {
    notifPersonal = [];
  ...
  const broadcastUnsub = onSnapshot(query(collection(db, NOTIFICATIONS), where("uid", "==", "all"), limit(50)), (snap) => {
    notifBroadcastAll = [];
...
function attachBroadcastListener() {
  ...
  const unsub = onSnapshot(q, (snap) => {
    if (snap.empty) { broadcastBoxEl.innerHTML = "📢 No broadcast available."; return; }
...
function renderReferralTree() {
  ...
  if (currentTreeView === "list") {
    referralTreeContainer.innerHTML = `<div class="table-responsive"><table class="table"><thead><tr><th>Level</th><th>Name</th><th>Referral Code</th><th>Status</th><th>Joined</th></tr></thead><tbody>${
      filtered.map((u) => `<tr>
        <td>Level ${u.level}</td><td>${u.name}</td><td>${u.referralCode}</td>
...
function renderReferralHistory() {
  ...
  referralHistoryTableEl.innerHTML = level1.length
    ? level1.map((u) => {
        ...
        const statusBadge = u.isActive ? '<span class="badge badge-success">Active</span>' : '<span class="badge badge-warning">Pending</span>';
        return `<tr>
          <td>${u.name}</td><td>${u.referralCode}</td><td>${formatDate(u.createdAt)}</td>
          <td>${u.activatedAt ? formatDate(u.activatedAt) : "--"}</td>
          <td>${formatCurrency(commission)}</td><td>${statusBadge}</td>
        </tr>`;
      }).join("")
...
function renderContestCard(contests) {
  ...
  if (!activeContest) { contestActiveCard.classList.add("hidden"); contestNoneCard.classList.remove("hidden"); return; }
  ...
  let percent = 0, statusText = "Upcoming", statusClass = "badge-warning";
  if (now < start) { percent = 0; statusText = "Upcoming"; statusClass = "badge-warning"; }
  else if (now > end) { percent = 100; statusText = "Ending Soon"; statusClass = "badge-danger"; }
  else { percent = Math.round(((now - start) / (end - start)) * 100); statusText = "Active"; statusClass = "badge-success"; }
  ...
  contestPrizeListEl.innerHTML = (activeContest.prizes || []).map((p) => `<span class="contest-prize-item">🏆 Rank ${p.rank}: ৳${p.amount}</span>`).join("");
}
...
function attachContestLeaderboardListener(uid) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    contestLeaderboardTableEl.innerHTML = docs.length
      ? docs.map((u, i) => `<tr${u.uid === uid ? ' style="background:rgba(37,99,235,.15)"' : ""}><td>#${i + 1}</td><td>${u.name}</td><td>${u.contestReferralCount || 0}</td></tr>`).join("")
      : `<tr><td colspan="3">🏆 No Contest Data Yet</td></tr>`;
    const myIndex = docs.findIndex((u) => u.uid === uid);
    contestMyRankEl.textContent = myIndex >= 0 ? `#${myIndex + 1} (${docs[myIndex].contestReferralCount || 0} Referrals)` : "Top 10-এ নেই";
  });
  subscriptions.push(unsub);
}
...
copyReferralBtn.addEventListener("click", () => { copyReferralInputEl.value = referralLinkEl.value; openModal(copyModal); });
copyNowBtn.addEventListener("click", async () => { await copyText(copyReferralInputEl.value); showToast("Copied", "Referral link copied to clipboard.", "success"); closeModalEl(copyModal); });
closeCopyModal.addEventListener("click", () => closeModalEl(copyModal));
copyLinkBtn.addEventListener("click", async () => { await copyText(referralLinkEl.value); showToast("Copied", "Referral link copied to clipboard.", "success"); });

shareBtn.addEventListener("click", async () => {
  const link = referralLinkEl.value;
  if (navigator.share) {
    try { await navigator.share({ title: "Refer Earn & Microjob", text: "Join Refer Earn & Microjob using my referral link!", url: link }); } catch {}
  } else {
    await copyText(link);
...
refreshBtn.addEventListener("click", () => { showToast("Refreshing", "Reloading your latest data...", "info"); setTimeout(() => window.location.reload(), 600); });
leaderboardBtn.addEventListener("click", () => { $("leaderboard").scrollIntoView({ behavior: "smooth" }); });
...
submitChangePasswordBtn.addEventListener("click", async () => {
  ...
  if (!currentPassword) { showWarning("Current Password দিন।"); return; }
  if (!newPassword || newPassword.length < 6) { showWarning("New Password কমপক্ষে ৬ অক্ষরের হতে হবে।"); return; }
  if (newPassword !== confirmPassword) { showWarning("New Password এবং Confirm Password মিলছে না।"); return; }

  submitChangePasswordBtn.disabled = true;
  try {
    showActionLoading(true);
    const credential = EmailAuthProvider.credential(phoneToPseudoEmail(currentUserData.phone), currentPassword);
...
saveProfileBtn.addEventListener("click", async () => {
  ...
  if (!name) { showWarning("Full name is required."); return; }

  const updateData = { name, district };
  if (currentUserData.authProvider !== "phone") updateData.phone = phone;
  if (selectedPhotoBase64) updateData.photoURL = selectedPhotoBase64;
...
profilePhotoEl.addEventListener("click", () => { previewImageEl.src = profilePhotoEl.src; openModal(imagePreviewModal); });
closePreview.addEventListener("click", () => closeModalEl(imagePreviewModal));

dashAvailableBalance.style.cursor = "pointer";
dashAvailableBalance.addEventListener("click", () => openModal(userInfoModal));
closeUserInfo.addEventListener("click", () => closeModalEl(userInfoModal));

document.querySelectorAll(".copy-number-btn").forEach((btn) => {
  btn.addEventListener("click", async () => { await copyText(btn.dataset.number); showToast("কপি হয়েছে", "নম্বর কপি হয়েছে", "success"); });
});

activationBtn.addEventListener("click", async () => {
  if (currentUserData.isActive) { showWarning("আপনার অ্যাকাউন্ট ইতিমধ্যে অ্যাক্টিভ আছে।"); return; }
  if (pendingActivationCount > 0) { showWarning("আপনার একটি অ্যাক্টিভেশন রিকোয়েস্ট ইতিমধ্যে Pending আছে।"); return; }

  const method = paymentMethodEl.value;
  const senderNumber = senderNumberEl.value.trim();
  const transactionId = transactionIdEl.value.trim();

  if (!senderNumber || !transactionId) { showWarning("সব ফিল্ড পূরণ করুন।"); return; }
  if (!BD_PHONE_REGEX.test(senderNumber)) { showWarning("সঠিক ১১ ডিজিটের মোবাইল নাম্বার দিন (যেমন 01XXXXXXXXX)।"); return; }
  ...
  try {
    ...
    await addDoc(collection(db, ACTIVATIONS), {
      uid: currentUser.uid, name: currentUserData.name, method, senderNumber, transactionId,
      status: "pending", requestedAt: serverTimestamp(), processedAt: null, adminNote: ""
    });
...
withdrawBtn.addEventListener("click", async () => {
  if (!currentUserData.isActive) { showWarning("Withdraw করার জন্য আপনার Account Active থাকতে হবে।"); return; }
  if (currentUserData.isFrozen) { showWarning("আপনার Account Frozen করা আছে, তাই Withdraw করা যাচ্ছে না।"); return; }
  if (pendingWithdrawCount > 0) { showWarning("আপনার একটি Withdraw Request ইতিমধ্যে Pending আছে। সেটি প্রসেস না হওয়া পর্যন্ত নতুন Request দেওয়া যাবে না।"); return; }
  ...
  if (balance < WITHDRAW_AMOUNT) { showWarning(`Withdraw করার জন্য Wallet Balance কমপক্ষে ৳${WITHDRAW_AMOUNT} হতে হবে।`); return; }
  if (cycleTasks < WITHDRAW_REQUIRED_APPROVED_TASKS) { showWarning(`Withdraw করতে কমপক্ষে ${WITHDRAW_REQUIRED_APPROVED_TASKS}টি New Task Approved প্রয়োজন, আপনার আছে ${cycleTasks}টি।`); return; }
  if (availableReferral < REQUIRED_NEW_REFERRAL) { showWarning(`Withdraw করতে ${REQUIRED_NEW_REFERRAL}টি New Active Referral প্রয়োজন, আপনার আছে ${availableReferral}টি।`); return; }
  if (!accountNumber) { showWarning("Mobile Number দিন।"); return; }
  if (!BD_PHONE_REGEX.test(accountNumber)) { showWarning("সঠিক ১১ ডিজিটের মোবাইল নাম্বার দিন (যেমন 01XXXXXXXXX)।"); return; }
  ...
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      const freshCycleTasks = freshData.cycleTaskCount || 0;

      if (freshBalance < WITHDRAW_AMOUNT) throw new Error("Insufficient wallet balance.");
      if (freshCycleTasks < WITHDRAW_REQUIRED_APPROVED_TASKS) throw new Error("Task requirement not met.");
      if (freshAvailable < REQUIRED_NEW_REFERRAL) throw new Error("Referral requirement not met.");

      transaction.update(userRef, {
        walletBalance: increment(-WITHDRAW_AMOUNT),
        withdrawableReferralCount: increment(-REQUIRED_NEW_REFERRAL),
        cycleTaskCount: increment(-WITHDRAW_REQUIRED_APPROVED_TASKS)
      });
      ...
      transaction.set(withdrawRef, {
        ...
        amount: WITHDRAW_AMOUNT, method, accountNumber,
        walletBalanceAtRequest: freshBalance, referralConsumed: REQUIRED_NEW_REFERRAL,
        taskConsumed: WITHDRAW_REQUIRED_APPROVED_TASKS,
        status: "pending", requestedAt: serverTimestamp(), processedAt: null, adminNote: ""
      });

      const txnRef = doc(collection(db, TRANSACTIONS));
      transaction.set(txnRef, {
        uid: currentUser.uid, type: "withdraw", amount: -WITHDRAW_AMOUNT,
        description: `Withdraw request via ${method}`, status: "pending", createdAt: serverTimestamp()
      });

      const notifRef = doc(collection(db, NOTIFICATIONS));
      transaction.set(notifRef, {
        uid: currentUser.uid, title: "Withdraw Request Submitted",
...
spinBtn.addEventListener("click", async () => {
  ...
  const today = getTodayDateStr();
  if (currentUserData.lastSpinDate === today) { showWarning("আজকের Lucky Spin ইতিমধ্যে সম্পন্ন হয়েছে।"); return; }
  ...
  setTimeout(async () => {
    try {
      ...
      const granted = await runTransaction(db, async (transaction) => {
        ...
        transaction.update(userRef, { walletBalance: increment(reward), totalIncome: increment(reward), lastSpinDate: today });
        const txnRef = doc(collection(db, TRANSACTIONS));
        transaction.set(txnRef, { uid: currentUser.uid, type: "lucky_spin", amount: reward, description: "Lucky Spin Reward", status: "completed", createdAt: serverTimestamp() });
        const notifRef = doc(collection(db, NOTIFICATIONS));
        transaction.set(notifRef, {
          uid: currentUser.uid, title: "Lucky Spin",
          message: reward > 0 ? `Lucky Spin থেকে আপনি ৳${reward} জিতেছেন!` : "আজকের Lucky Spin-এ এবার কোনো Reward আসেনি, কাল আবার চেষ্টা করুন।",
          type: reward > 0 ? "success" : "info", isRead: false, createdAt: serverTimestamp()
        });
        return true;
      });

      if (granted) {
        spinResultEl.textContent = reward > 0 ? `🎉 আপনি ৳${reward} জিতেছেন!` : "😔 এবার কোনো Reward আসেনি। কাল আবার চেষ্টা করুন।";
        if (reward > 0) showSuccess(`অভিনন্দন! আপনি Lucky Spin থেকে ৳${reward} জিতেছেন।`);
        else showToast("Lucky Spin", "এবার কোনো Reward আসেনি। কাল আবার চেষ্টা করুন।", "info");
      } else {
...
quickNavBtn.addEventListener("click", () => { quickNavReferralLink.value = referralLinkEl.value; quickNavOverlay.classList.remove("hidden"); });
closeQuickNav.addEventListener("click", () => quickNavOverlay.classList.add("hidden"));
quickNavOverlay.addEventListener("click", (e) => { if (e.target === quickNavOverlay) quickNavOverlay.classList.add("hidden"); });
quickNavCopyBtn.addEventListener("click", async () => { await copyText(quickNavReferralLink.value); showToast("Copied", "Referral link copied to clipboard.", "success"); });
