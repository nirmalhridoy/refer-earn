...
function renderActiveTaskList() {
  ...
  const list = allActiveTasks.filter((t) => t.platform === activeCategory);
  let unreadCount = 0;
  ...
  taskListContainer.innerHTML = list.length
    ...
    : `<div class="card">📋 No Available Tasks<br>New tasks will appear when available.</div>`;
...
function updateWithdrawUI() {
  ...
  const condition1Met = balance >= WITHDRAW_AMOUNT && approvedTasks >= WITHDRAW_REQUIRED_APPROVED_TASKS;

  withdrawInsufficientCard.classList.toggle("hidden", condition1Met);
  withdrawFormCard.classList.toggle("hidden", !condition1Met);

  if (!condition1Met) {
    const reasons = [];
    if (balance < WITHDRAW_AMOUNT) reasons.push(`Wallet Balance কমপক্ষে ৳${WITHDRAW_AMOUNT} হতে হবে (বর্তমান: ${formatCurrency(balance)})`);
    if (approvedTasks < WITHDRAW_REQUIRED_APPROVED_TASKS) reasons.push(`কমপক্ষে ${WITHDRAW_REQUIRED_APPROVED_TASKS}টি Task Approved হতে হবে (বর্তমান: ${approvedTasks})`);
    withdrawInsufficientCard.innerHTML = `<p>💰 Withdraw করতে যা প্রয়োজন:</p>` + reasons.map((r) => `<p>• ${r}</p>`).join("");
    return;
  }
  ...
  if (availableReferral >= REQUIRED_NEW_REFERRAL) {
    ...
    withdrawReferralStatusEl.innerHTML = `<span class="badge badge-warning">❌ 3 New Active Referral প্রয়োজন (আছে ${availableReferral}টি)</span>`;
  }

  withdrawTaskRequirementStatusEl.innerHTML = `<span class="badge badge-success">✅ ${WITHDRAW_REQUIRED_APPROVED_TASKS} Approved Task সম্পন্ন (${approvedTasks}/${WITHDRAW_REQUIRED_APPROVED_TASKS})</span>`;
}
...
function attachWithdrawListener(uid) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    withdrawHistoryEl.innerHTML = docs.length
      ...
      : `<tr><td colspan="4">💸 No Withdrawal History Yet</td></tr>`;
