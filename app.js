...
function updateWithdrawUI() {
  ...
  if (availableReferral >= REQUIRED_NEW_REFERRAL) {
    withdrawReferralStatusEl.innerHTML = `<span class="badge badge-success">✅ Referral Requirement Met</span>`;
  } else {
    withdrawReferralStatusEl.innerHTML = `<span class="badge badge-warning">❌ Referral Requirement Not Met</span>`;
  }
  ...
  if (approvedTasks >= WITHDRAW_REQUIRED_APPROVED_TASKS) {
    ...
    withdrawTaskRequirementStatusEl.innerHTML = `<span class="badge badge-danger">❌ ${WITHDRAW_REQUIRED_APPROVED_TASKS}টি Approved Task প্রয়োজন (আছে ${approvedTasks}/${WITHDRAW_REQUIRED_APPROVED_TASKS})</span>`;
  }
}
...
function attachTodayIncomeListener(uid) {
  ...
  const unsub = onSnapshot(q, (snap) => {
    ...
    snap.forEach((d) => {
      const t = d.data();
      if (["referral_bonus", "activation_bonus", "admin_credit", "daily_login_bonus", "lucky_spin", "rank_bonus", "task_reward", "contest_prize", "registration_bonus"].includes(t.type)) {
...
/* ===== NOTIFICATIONS (Fixed: 2 separate listeners, no composite index needed) ===== */
...
withdrawBtn.addEventListener("click", async () => {
  ...
  if (availableReferral < REQUIRED_NEW_REFERRAL) {
    showWarning(`Withdraw করার শর্ত এখনো পূরণ হয়নি (Referral Requirement)।`);
    return;
  }
  if (approvedTasks < WITHDRAW_REQUIRED_APPROVED_TASKS) {
