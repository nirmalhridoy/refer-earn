...
async function openUserDetails(uid) {
  try {
    ...
    udModalReferrals.textContent = u.referralCount || 0;
...
udActivateBtn.addEventListener("click", async () => {
  ...
  const confirmed = await showConfirm(
    ...
    `Are you sure you want to ${newActiveState ? "activate" : "deactivate"} ${selectedUser.name}'s account? This is a manual override and will not trigger referral bonus.`
  );
  if (!confirmed) return;
  ...
  try {
    ...
    if (decision === "approved") {
      ...
      if (userData && userData.referredBy) {
        const refQuery = query(collection(db, USERS), where("referralCode", "==", userData.referredBy), limit(1));
        const refSnap = await getDocs(refQuery);
        if (!refSnap.empty) {
          referrerRef = doc(db, USERS, refSnap.docs[0].id);
        }
      }
    }
      ...
      if (decision === "approved") {
        ...
        if (referrerRef) {
          transaction.update(referrerRef, {
            ...
            totalIncome: increment(REFERRAL_BONUS_AMOUNT),
            referralEarnings: increment(REFERRAL_BONUS_AMOUNT),
            referralCount: increment(1)
          });

          const bonusTxnRef = doc(collection(db, TRANSACTIONS));
          transaction.set(bonusTxnRef, {
            uid: referrerRef.id,
            type: "referral_bonus",
            amount: REFERRAL_BONUS_AMOUNT,
            description: `Referral bonus for activating ${a.name}`,
            status: "completed",
            createdAt: serverTimestamp()
          });

          const referrerNotifRef = doc(collection(db, NOTIFICATIONS));
          transaction.set(referrerNotifRef, {
            uid: referrerRef.id,
            title: "Referral Bonus Received",
            message: `You earned ${formatCurrency(REFERRAL_BONUS_AMOUNT)} because ${a.name} activated their account.`,
            type: "success",
            isRead: false,
            createdAt: serverTimestamp()
          });
