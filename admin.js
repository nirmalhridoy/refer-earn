...
function attachTaskManagementListeners() {
  const fbUnsub = onSnapshot(query(collection(db, TASKS), where("platform", "==", "facebook"), orderBy("createdAt", "desc")), (snap) => {
    const rows = [];
  ...
  const ytUnsub = onSnapshot(query(collection(db, TASKS), where("platform", "==", "youtube"), orderBy("createdAt", "desc")), (snap) => {
    const rows = [];
    snap.forEach((d) => { const t = { id: d.id, ...d.data() }; allTasksById[t.id] = t; rows.push(t); });
...
function attachSubmissionsListener() {
  ...
  const q = constraints.length
    ? query(collection(db, TASK_SUBMISSIONS), ...constraints, orderBy("submittedAt", "desc"), limit(50))
    : query(collection(db, TASK_SUBMISSIONS), orderBy("submittedAt", "desc"), limit(50));
