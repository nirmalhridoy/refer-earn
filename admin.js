import {
  ...
  ADMIN_EMAILS, RANK_LEVELS, LEVEL_COMMISSIONS, TASK_PLATFORMS, WITHDRAW_REQUIRED_APPROVED_TASKS
} from "./firebase.js";
...
const deleteUserConfirmInput = $("deleteUserConfirmInput");
const deleteUserSubmitBtn = $("deleteUserSubmitBtn");
const deleteUserCancelBtn = $("deleteUserCancelBtn");
...
async function openUserDetails(uid) {
  try {
    ...
    udModalApprovedTasks.textContent = `Lifetime: ${u.approvedTaskCount || 0} • This Cycle: ${u.cycleTaskCount || 0}/${WITHDRAW_REQUIRED_APPROVED_TASKS}`;
...
udDeleteBtn.addEventListener("click", () => {
  if (!selectedUser) return;
  deleteUserName.textContent = `${selectedUser.name} (${selectedUser.email || selectedUser.phone})`;
  deleteUserConfirmInput.value = "";
  closeModalEl(userDetailsModal);
  openModal(deleteUserModal);
});

deleteUserCancelBtn.addEventListener("click", () => closeModalEl(deleteUserModal));

deleteUserSubmitBtn.addEventListener("click", async () => {
  if (!selectedUser) return;

  if (deleteUserConfirmInput.value.trim() !== "DELETE") {
    showWarning('নিশ্চিত করতে অবশ্যই "DELETE" টাইপ করতে হবে।');
    return;
  }
  ...
  try {
    ...
    const [txnSnap, withdrawSnap, activationSnap, submissionSnap, notifSnap] = await Promise.all([
      getDocs(query(collection(db, TRANSACTIONS), where("uid", "==", uid))),
      getDocs(query(collection(db, WITHDRAWS), where("uid", "==", uid))),
      getDocs(query(collection(db, ACTIVATIONS), where("uid", "==", uid))),
      getDocs(query(collection(db, TASK_SUBMISSIONS), where("uid", "==", uid))),
    ...
    activationSnap.forEach((d) => batch.delete(d.ref));
    submissionSnap.forEach((d) => batch.delete(d.ref));
    notifSnap.forEach((d) => batch.delete(d.ref));

    await batch.commit();

    closeModalEl(deleteUserModal);
    showSuccess("User account ও সংশ্লিষ্ট সব Data মুছে ফেলা হয়েছে।");
  } catch (err) {
    showError(err.message);
  } finally {
    showActionLoading(false);
...
function attachWithdrawTableListener() {
  ...
  // Fixed: removed orderBy() paired with where() to avoid composite index requirement — sort client-side
  let q;

  if (filterValue === "all") {
    q = query(collection(db, WITHDRAWS), limit(200));
  } else {
    q = query(collection(db, WITHDRAWS), where("status", "==", filterValue), limit(200));
  }
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (decision === "rejected") {
        ...
        transaction.update(userRef, {
          ...
          cycleTaskCount: increment(w.taskConsumed || WITHDRAW_REQUIRED_APPROVED_TASKS)
        });
      }
      ...
      transaction.set(notifRef, {
        ...
        message: decision === "approved"
          ...
          : `আপনার ${formatCurrency(w.amount)} Withdraw Request Reject হয়েছে এবং Amount, Referral ও Task Count Wallet-এ ফেরত দেওয়া হয়েছে।`,
...
function attachActivationTableListener() {
  ...
  if (filterValue === "all") {
    q = query(collection(db, ACTIVATIONS), limit(200));
  } else {
    q = query(collection(db, ACTIVATIONS), where("status", "==", filterValue), limit(200));
  }
        ...
        for (let i = 0; i < referralChain.length; i++) {
          ...
          if (rankChangedTo && rankBonusAmount > 0) {
            ...
            transaction.set(rankNotifRef, {
              uid: referralChain[i].uid,
              title: "Rank Upgrade!",
              message: `অভিনন্দন! আপনি ${rankChangedTo} Rank-এ Upgrade হয়েছেন এবং ৳${rankBonusAmount} Bonus পেয়েছেন। এই Rank আপনার Total Active Referral Count থেকে Automatically নির্ধারিত হয়েছে।`,
              type: "success",
              isRead: false,
              createdAt: serverTimestamp()
            });
